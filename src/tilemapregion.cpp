...
TilemapRegion::TilemapRegion() { this->draw_debug = Engine::GetInstance().config->debug.draw_map_regions; this->state = SpriteState::Active; }

void TilemapRegion::Render(SDL_Renderer * renderer, const CB_ViewClippingInfo & clip) {
    if (this->draw_debug && clip.z_index == ZIndex::Foreground) {
