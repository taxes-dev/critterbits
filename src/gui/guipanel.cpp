...
CB_ViewClippingInfo GuiPanel::AdjustClipToClientArea(const CB_ViewClippingInfo & clip_info, const CB_Rect & control_dim) const {
    CB_ViewClippingInfo client_area = clip_info;
    client_area.source = control_dim;
    client_area.dest.x += control_dim.x;
    client_area.dest.y += control_dim.y;
...
void GuiPanel::Reflow(const CB_Rect & parent_rect) {
    ...
    for (auto & control : this->children) {
        ...
        int col = control->grid.x;
        control->dim.x = 0;
        std::for_each(row_widths.begin(), row_widths.begin() + row, [&control](int w) {
            control->dim.x += w;
        });
        control->dim.y = 0;
        std::for_each(col_heights.begin(), col_heights.begin() + col, [&control](int h) {
            control->dim.y += h;
        });
